        ...
        private BiteVmInterpretResult Run()
        {
            while (true)
            {
                        ...
                        case SrslVmOpCodes.OpBitwiseAnd:
                            {
                                ...
                                if ((valueRhs.DynamicType < DynamicVariableType.True) &&
                                    (valueLhs.DynamicType < DynamicVariableType.True))
                                {
                                    var lhs = (int)valueLhs.NumberData;
                                    var rhs = (int)valueRhs.NumberData;
                                    m_VmStack.Push(DynamicVariableExtension.ToDynamicVariable(lhs & rhs));
                                }
                                else
                                {
                                    throw new Exception("Can only perform bitwise operations on Integers and Floating Point Numbers!");
                                }
                        ...
                        case SrslVmOpCodes.OpAffirm:
                            {
                                ...
                                {
                                    throw new Exception("Can only affirm Integers and Floating Point Numbers!");
                                }
                        ...
                        case SrslVmOpCodes.OpPrefixDecrement:
                            {
                                ...
                                if (currentStack.DynamicType < DynamicVariableType.True)
                                {
                                    currentStack.NumberData = currentStack.NumberData - 1;
                                }
                                else
                                {
                ...
                else
                {
                    ...
                    {
                        // Allow expressions to return their results
